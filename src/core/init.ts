...
const WORKFLOWS_FOR_CLEANUP = [...ALL_WORKFLOWS, 'verify'] as const;
...
export class InitCommand {
  ...
  private async generateSkillsAndCommands(
    ...
    removedDeselectedCommandCount: number;
    removedDeselectedSkillCount: number;
  }> {
    ...
    for (const tool of tools) {
      ...
      try {
        ...
        if (shouldGenerateSkills) {
          ...
          removedDeselectedSkillCount += await this.removeUnselectedSkillDirs(skillsDir, workflows);
        }
        if (!shouldGenerateSkills) {
          const skillsDir = path.join(projectPath, tool.skillsDir, 'skills');
  ...
  private displaySuccessMessage(
    ...
    if (results.removedDeselectedCommandCount > 0) {
      console.log(chalk.dim(`Removed: ${results.removedDeselectedCommandCount} command files (deselected workflows)`));
    }
    if (results.removedDeselectedSkillCount > 0) {
      console.log(chalk.dim(`Removed: ${results.removedDeselectedSkillCount} skill directories (deselected workflows)`));
    }
    ...
    // Getting started (task 7.6: show the first planning workflow in profile)
    const globalCfg = getGlobalConfig();
    const activeProfile: Profile = (this.profileOverride as Profile) ?? globalCfg.profile ?? 'core';
    const activeWorkflows = [...getProfileWorkflows(activeProfile, globalCfg.workflows)];
    console.log();
    if (activeWorkflows.includes('explore')) {
      console.log(chalk.bold('Getting started:'));
      console.log('  Explore the change first: /opsx:explore "your idea"');
  ...
  private async removeSkillDirs(skillsDir: string): Promise<number> {
    ...
    for (const workflow of WORKFLOWS_FOR_CLEANUP) {
