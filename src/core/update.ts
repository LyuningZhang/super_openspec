...
const OLD_CORE_WORKFLOWS = ['explore', 'propose', 'apply', 'archive'] as const;
...
export class UpdateCommand {
  ...
  private displayUpdatePlan(
    ...
    const updates = toolsToUpdate.map((toolId) => {
      ...
      if (status?.needsUpdate) {
        ...
        return `${status.toolId} (${fromVersion} â†?${OPENSPEC_VERSION})`;
      }
  ...
  private displayOldCoreCustomProfileNote(profile: Profile, workflows?: readonly string[]): void {
    ...
    console.log(chalk.dim('Note: The core profile now includes review and sync. Your custom profile is preserving the old core workflow set.'));
    console.log(chalk.dim('Run `openspec config profile core` and then `openspec update` to add review and sync.'));
  }
  ...
  private async removeUnselectedSkillDirs(
    ...
    const desiredSet = new Set<string>(desiredWorkflows);
    let removed = 0;
