...
describe('InitCommand', () => {
  ...
  describe('execute with --tools flag', () => {
    ...
    it('should create core profile skills for Claude Code by default', async () => {
      ...
      // Core profile: explore, propose, apply, review, sync, archive
...
describe('InitCommand - profile and detection features', () => {
  ...
  it('should remove deselected workflow artifacts on re-init', async () => {
    saveGlobalConfig({
      featureFlags: {},
      profile: 'core',
      delivery: 'both',
    ...
    const reviewSkill = path.join(testDir, '.claude', 'skills', 'openspec-review-change', 'SKILL.md');
    const reviewCommand = path.join(testDir, '.claude', 'commands', 'opsx', 'review.md');
    expect(await fileExists(reviewSkill)).toBe(true);
    expect(await fileExists(reviewCommand)).toBe(true);
    ...
    expect(await fileExists(reviewSkill)).toBe(false);
    expect(await fileExists(reviewCommand)).toBe(false);
  });

  it('should remove legacy verify artifacts on init', async () => {
    await fs.mkdir(path.join(testDir, '.claude', 'skills', 'openspec-verify-change'), { recursive: true });
    await fs.writeFile(
    ...
    await fs.writeFile(path.join(testDir, '.claude', 'commands', 'opsx', 'verify.md'), '# legacy verify command\n');

    const initCommand = new InitCommand({ tools: 'claude', force: true });
    await initCommand.execute(testDir);

    expect(await fileExists(path.join(testDir, '.claude', 'skills', 'openspec-review-change', 'SKILL.md'))).toBe(true);
    expect(await fileExists(path.join(testDir, '.claude', 'commands', 'opsx', 'review.md'))).toBe(true);
    expect(await fileExists(path.join(testDir, '.claude', 'skills', 'openspec-verify-change', 'SKILL.md'))).toBe(false);
    expect(await fileExists(path.join(testDir, '.claude', 'commands', 'opsx', 'verify.md'))).toBe(false);
  });
