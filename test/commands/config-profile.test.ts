...
describe('diffProfileState workflow formatting', () => {
  ...
  it('uses explicit labels when workflows are added and removed', async () => {
    ...
    const diff = diffProfileState(
      { profile: 'custom', delivery: 'both', workflows: ['propose', 'sync'] },
      { profile: 'custom', delivery: 'both', workflows: ['propose', 'review'] },
    );

    expect(diff.hasChanges).toBe(true);
    expect(diff.lines).toEqual(['workflows: added review; removed sync']);
  });
});
...
describe('deriveProfileFromWorkflowSelection', () => {
  ...
  it('returns custom when selection is a superset of core workflows', async () => {
    const { deriveProfileFromWorkflowSelection } = await import('../../src/commands/config.js');
    expect(deriveProfileFromWorkflowSelection(['explore', 'propose', 'apply', 'sync', 'archive', 'new'])).toBe('custom');
  });

  it('returns core when selection has exactly core workflows in different order', async () => {
    const { deriveProfileFromWorkflowSelection } = await import('../../src/commands/config.js');
    expect(deriveProfileFromWorkflowSelection(['archive', 'sync', 'review', 'apply', 'explore', 'propose'])).toBe('core');
  });
});
...
describe('config profile interactive flow', () => {
  ...
  function setupSyncedCoreBothArtifacts(projectDir: string): void {
    ...
    const coreCommands = ['explore', 'propose', 'apply', 'review', 'sync', 'archive'];
  ...
  function addExtraOnboardWorkflowArtifacts(projectDir: string): void {
    const onboardSkillPath = path.join(projectDir, '.claude', 'skills', 'openspec-onboard', 'SKILL.md');
    fs.mkdirSync(path.dirname(onboardSkillPath), { recursive: true });
    fs.writeFileSync(onboardSkillPath, 'name: openspec-onboard\n', 'utf-8');

    const onboardCommandPath = path.join(projectDir, '.claude', 'commands', 'opsx', 'onboard.md');
    fs.mkdirSync(path.dirname(onboardCommandPath), { recursive: true });
    fs.writeFileSync(onboardCommandPath, '# onboard\n', 'utf-8');
  }
  ...
  it('delivery-only action should not invoke workflow checkbox prompt', async () => {
    ...
    saveGlobalConfig({ featureFlags: {}, profile: 'core', delivery: 'both', workflows: ['explore', 'propose', 'apply', 'review', 'sync', 'archive'] });
  ...
  it('workflows-only action should not invoke delivery prompt', async () => {
    ...
    checkbox.mockResolvedValueOnce(['explore', 'propose']);
    ...
    expect(getGlobalConfig().workflows).toEqual(['explore', 'propose']);
  });
  ...
  it('workflow picker should use friendly names with descriptions', async () => {
    ...
    saveGlobalConfig({ featureFlags: {}, profile: 'core', delivery: 'both', workflows: ['explore', 'propose', 'apply', 'review', 'sync', 'archive'] });
    select.mockResolvedValueOnce('workflows');
    checkbox.mockResolvedValueOnce(['explore', 'propose', 'apply', 'review', 'sync', 'archive']);
    ...
    expect(checkboxCall.choices).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({
        value: 'review',
        name: 'Review change',
        description: 'Review implementation against artifacts without archiving',
      }),
    ]));
  });
  ...
  it('keep action should warn when project has extra workflows beyond global config', async () => {
    ...
    saveGlobalConfig({ featureFlags: {}, profile: 'core', delivery: 'both', workflows: ['explore', 'propose', 'apply', 'review', 'sync', 'archive'] });
    setupSyncedCoreBothArtifacts(tempDir);
    addExtraOnboardWorkflowArtifacts(tempDir);
    select.mockResolvedValueOnce('keep');
  ...
  it('core preset should preserve delivery setting', async () => {
    ...
    expect(config.workflows).toEqual(['explore', 'propose', 'apply', 'review', 'sync', 'archive']);
